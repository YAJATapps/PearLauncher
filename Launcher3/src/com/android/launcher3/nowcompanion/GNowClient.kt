...
class GNowClient(private val launcher: Launcher) : ILauncherClient {
    ...
    override fun onStart() {
        if (!destroyed && FeatureFlags.GOOGLE_NOW) {
    ...
    private inline fun ifConnected(body: () -> Unit) {
        if (!isConnected || !FeatureFlags.GOOGLE_NOW) {
            return
        }
    ...
    private inner class ProxyCallbacks : ILauncherClientProxyCallback.Stub(), Handler.Callback {
        private var mClient: GNowClient? = null
        ...
        fun setClient(client: GNowClient) {
            mClient = client
    ...
    companion object {
        const val PROXY_PACKAGE = "com.pearlauncher.nowcompanion"
        const val PROXY_SERVICE = ".LauncherClientProxyService"
        const val TAG = "GNowClient"
