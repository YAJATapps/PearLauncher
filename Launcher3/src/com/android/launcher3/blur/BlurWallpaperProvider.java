...
public class BlurWallpaperProvider {
    ...
    public BlurWallpaperProvider(Context context) {
        ...
        sEnabled = mWallpaperManager.getWallpaperInfo() == null && FeatureFlags.BLUR;
        updateBlurRadius();
    }

    private void updateBlurRadius() {
        mBlurRadius = (75) / mDownsampleFactor);
        mBlurRadius = Math.max(1, Math.min(mBlurRadius, 25));
    }

    public void updateWallpaper() {
        Launcher launcher = Launcher.getLauncher(mContext);
        boolean enabled = mWallpaperManager.getWallpaperInfo() == null && FeatureFlags.BLUR;
        if (enabled != sEnabled) {
    ...
    public int getTintColor() {
        return ContextCompat.getColor(mContext, R.color.blurTintColor);
    }
    ...
    public static BlurWallpaperProvider getInstance() {
        return Launcher.getLauncher(sContext).getBlurWallpaperProvider();
    }
}
